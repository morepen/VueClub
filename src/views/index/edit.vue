                ...
                <el-col :span="24" class="warp-main">
                    <el-form ref="infoForm" :model="infoForm" :rules="rules" label-width="50px">
                        <el-form-item label="标题" prop="a_title">
                          ...
                          <el-form-item label="名言" prop="a_title">
                            <el-select  v-model="goldword"  @change="handleword($event)">
                                <el-option
  ...
  export default{
    ...
    GetDetail(){
        ...
        this.$http.baseAction("GetArticleDetail",params)
            .then(response => {
                ...
                that.infoForm.a_title=that.ArticleDetail.title;
                that.tagname=that.ArticleDetail.tag;
                that.infoForm.a_type=that.ArticleDetail.tag;
                that.tagid=that.ArticleDetail.tagid;
                that.infoForm.imageUrl=that.ArticleDetail.imageUrl;
                that.content=that.ArticleDetail.content;
                that.goldword=that.ArticleDetail.glodword;
                that.wordfrom=that.ArticleDetail.wordfrom;
                that.formattedAddress=that.ArticleDetail.address;
                that.isshowmap=false;
                that.infoForm.imageUrl=that.ArticleDetail.coverimage;
     ...
     handleword(event){
       ...
       for(var i=0;i<this.wordlist.length;i++){
          var _obj=this.wordlist[i];
          if(event==_obj.word){
             this.wordfrom=_obj.wordfrom
          }
       }
     ...
     GetWordList(){
        ...
        this.$http.baseAction("WordArticle",params).then(function (response) {
                    
                    if(response.code=="200"){
                       
                         that.wordlist=response.data; 
                         that.wordfrom=response.data[0].wordfrom;
                         that.goldword=response.data[0].word;
    ...
    onSubmit:function(event){
        ...
        var params={
          ...
          goldword:this.goldword,
          wordfrom:this.wordfrom,
          id:this.$route.query.id
        };
       
        Loading.service({text:"Loading..."});
        this.$http.baseAction("UpdateArticle",params).then(function (response) {
